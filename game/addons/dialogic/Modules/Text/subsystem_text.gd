...
func color_character_names(text:String) -> String:
	...
	for result in color_regex.search_all(text):
		var character_color: Color = character_colors.get(result.get_string('name'), Color.WHITE)
		text = text.insert(result.get_start("name")+((9+8+8)*counter), '[color=#' + character_color.to_html() + ']')
