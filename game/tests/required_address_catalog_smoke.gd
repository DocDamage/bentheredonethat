...
func _ready() -> void:
	...
	for address_id in CATALOG.ADDRESS_ORDER:
		...
		total += rooms.size()
		assert(not bool(definition.get("runtimeEnabled", true)), "%s must remain gated until source admission and room implementation." % address_id)
	...
	print("REQUIRED_ADDRESS_CATALOG_SMOKE_OK addresses=6 rooms=64 budgets=12+12+12+10+10+8 runtime_gated=true")
	get_tree().quit(0)
