...
func _ready() -> void:
	...
	assert((af01.get("navigation", {}) as Dictionary).get("collisionState", &"") == &"native_perimeter_shapes_authored")
	assert(FileAccess.file_exists(String((af01.get("layout", {}) as Dictionary).get("firstVisitCapture", ""))))
	print("ADDRESS_ROOM_RECORDS_SMOKE_OK records=1 room=AF-01 state=scene_collision_authored_runtime_gated")
	get_tree().quit(0)
