...
func _ready() -> void:
	...
	assert(bool(contract.get("runtimeEnabled", false)))
	...
	print("ADDRESS_POPULATION_CATALOG_SMOKE_OK room=AF-01 residents=2 visitor=iron_sentinel runtime_admitted=true")
	get_tree().quit(0)
