	...
	if not Engine.is_editor_hint():
		...
		_gamepiece.direction_changed.connect(
			func _on_gamepiece_direction_changed(new_direction: Directions.Points):
				var offset: Vector2 = Directions.MAPPINGS[new_direction] * Gameboard.properties.cell_size
...
func _unhandled_input(event: InputEvent) -> void:
	# InputMap normalizes keyboards, D-pads, and analog sticks into the same
	# actions. Do not gate this on InputEventKey or controllers cannot initiate
	# movement from rest.
