@tool
class_name NeonViperFieldAnimation
extends GamepieceAnimation

const FRAME_RATE := 9.0
const RUN_FRAME_COUNT := 6
const ASSET_ROOT := "res://game_assets/characters/Recruitable Characters/neon viper - cyberpunk female"
...
func _update_texture() -> void:
	...
	if _sequence == "run":
		var horizontal := "west" if direction == Directions.Points.WEST or direction == Directions.Points.NORTH else "east"
		path = "%s/animations/Running-a938bf16/%s/frame_%03d.png" % [ASSET_ROOT, horizontal, _frame]
