@tool
class_name RiftJackalFieldAnimation
extends GamepieceAnimation

const FRAME_RATE := 8.0
const ASSET_ROOT := "res://game_assets/characters/Topdown Monsters Part 1/Sliced/Rift Jackal"
const SEQUENCES := {
...
func set_direction(value: Directions.Points) -> void:
	direction = value
	# The source monster is authored front-facing. A subtle flip keeps east/west
	# turns readable without pretending that the pack contains directional art.
