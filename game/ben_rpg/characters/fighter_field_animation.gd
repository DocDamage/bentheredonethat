@tool
class_name FighterFieldAnimation
extends GamepieceAnimation

const FRAME_RATE := 9.0
const WALK_FRAME_COUNT := 6
const ASSET_ROOT := "res://game_assets/characters/Recruitable Characters/Fighter/Fighter"
...
func _process(delta: float) -> void:
	if _sequence != "run":
		return
	_elapsed += delta
	var next_frame := int(_elapsed * FRAME_RATE) % WALK_FRAME_COUNT
...
func _update_texture() -> void:
	...
	if _sequence == "run":
		path = "%s/animations/Confident_Walk/%s/frame_%03d.png" % [ASSET_ROOT, facing, _frame]
