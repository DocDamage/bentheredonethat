class_name CampaignNewPhiladelphiaFranklinLaboratory
extends Node2D
...
func _ready() -> void:
	...
	configure(ROOM_RECORDS.record(&"NP-01"))


func configure(record_definition: Dictionary) -> void:
	_record = record_definition.duplicate(true)
	name = "AuthoredNewPhiladelphiaRoom_NP-01"
	set_meta(&"room_id", &"NP-01")
...
func _install_feature_marker() -> void:
	...
	marker.name = "NewPhiladelphiaFeature_FranklinWorkbench"
	marker.position = Vector2(15, 10) * 48.0
	marker.set_meta(&"feature_id", &"franklin_workbench")
