...
func _scripted_encounter(local: Vector2i) -> StringName:
	if Rect2i(0, 4, 8, 4).has_point(local) and not CampaignState.story_flags.get(&"primeval_grove_cleared", false): return &"primeval_grove_intro"
...
func _is_danger_region(local: Vector2i) -> bool:
	return Rect2i(10, 4, 8, 4).has_point(local) or Rect2i(20, 4, 8, 4).has_point(local) or _in_nest(local) or _in_caldera(local)
...
func _in_nest(local: Vector2i) -> bool: return Rect2i(10, 14, 8, 4).has_point(local)
func _in_caldera(local: Vector2i) -> bool: return Rect2i(20, 14, 8, 4).has_point(local)
