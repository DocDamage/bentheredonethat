...
func _scripted_encounter(local: Vector2i) -> StringName:
	if Rect2i(0, 4, 8, 4).has_point(local) and not CampaignState.story_flags.get(&"moonpetal_gate_cleared", false):
		return &"moonpetal_gate_intro"
	if Rect2i(10, 14, 8, 4).has_point(local) and not CampaignState.story_flags.get(&"moonpetal_bell_ambush_cleared", false):
		return &"moonpetal_bell_ambush"
	if Rect2i(20, 14, 8, 4).has_point(local) and local.x >= 23 and CampaignState.story_flags.get(&"moonpetal_palace_open", false) and not CampaignState.story_flags.get(&"moonpetal_scenario_complete", false):
