class_name CampaignNewPhiladelphiaPowerRecordsBasementGround
extends Node2D
...
func _ready() -> void:
	...
	for profile_id in [&"laboratory_floor_tile", &"laboratory_wall_tile"]: _textures[profile_id] = _profiles.texture(profile_id)
...
func _draw() -> void:
	...
	for y in range(dimensions.y):
		for x in range(dimensions.x):
			...
			if texture: draw_texture_rect_region(texture, Rect2(Vector2(x, y) * 48.0, _profiles.world_draw_size(profile_id)), _profiles.region(profile_id))
