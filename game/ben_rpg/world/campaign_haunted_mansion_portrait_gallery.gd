class_name CampaignHauntedMansionPortraitGallery
extends Node2D
...
var _upper_left_frame: Texture2D
var _upper_right_frame: Texture2D
...
func _ready() -> void:
	...
	_upper_left_frame = _profiles.texture(&"mansion_gallery_upper_left_frame")
	_upper_right_frame = _profiles.texture(&"mansion_gallery_upper_right_frame")
...
func _draw() -> void:
	...
	draw_rect(bounds, Color("17101b"), true)
	draw_rect(Rect2(Vector2(48, 4 * 48), Vector2((_dimensions.x - 2) * 48, (_dimensions.y - 5) * 48)), Color("4f3337"), true)
	for x in range(2, _dimensions.x - 1, 3):
		draw_line(Vector2(x * 48, 4 * 48), Vector2(x * 48, (_dimensions.y - 1) * 48), Color("2a1a24"), 4.0)
	...
	_draw_profile(&"mansion_gallery_upper_left_frame", _upper_left_frame, Vector2(7 * 48, 2 * 48))
	_draw_profile(&"mansion_gallery_upper_right_frame", _upper_right_frame, Vector2((_dimensions.x - 10) * 48, 2 * 48))
	_draw_profile(&"mansion_gallery_stage_curtain", _curtain, Vector2((_dimensions.x / 2.0 - 2.0) * 48, 5 * 48))
	draw_rect(bounds, Color("c49a9b"), false, 2.0)
