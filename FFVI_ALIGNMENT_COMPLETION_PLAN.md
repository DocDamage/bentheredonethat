...
| M1 — Visual foundation | In progress | `FIELD_SCALE_BIBLE.md`, 308 visual profiles, runtime inventory/provenance generators, layered field registry, eight foreground scripts, camera rounding, and focused smoke tests exist | Vertical-slice profile coverage is not 100%; 138/162 static runtime textures are profiled; native-scale review and camera/resolution sign-off remain open |
...
| M4 — Universe migration | Started early | Each universe has an expanded 8x4 room footprint/layout contract; all six universe foreground implementations, including Helios, exist; refreshed captures and layout tests exist | This work must not be called complete before M0 and the vertical-slice acceptance gates close; no universe has complete human sign-off |
